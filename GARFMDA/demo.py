...
def kflod_5(known,unknown,A):
    ...
    for cv in range(1, 6):
        ...
        score,tlabel= train11(F_train, label_train, F_test, tlabel_test)
        for i in range(len(score)):
            scores.append(score[i])
            tlabels.append(tlabel[i])
        print("fold cv--{}".format(cv))
    return scores,tlabels

def k(i):
  scores1,tlabels1 =kflod_5(known,unknown,A)
  scores2,tlabels2 = kflod_5(known1,unknown1,A1)
  scores1 = np.array(scores1)
  scores=[]
  for i in range(scores1.shape[0]):
        scores.append((scores1[i]+scores2[i])/2)

  fpr, tpr, threshold = roc_curve(tlabels1, scores)
  i = str(i)
  np.savetxt("./data/fpr_tpr/fpr"+i+".txt",fpr)
  np.savetxt("./data/fpr_tpr/tpr" + i + ".txt", tpr)
  auc_val = auc(fpr, tpr)
  print(auc_val)
  return auc_val
auc_val=[]
